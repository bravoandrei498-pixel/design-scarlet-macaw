...
- ☐ **P0** Grouping (CRAFT-RULES C13 — Gestalt): in-group spacing tighter than between-group spacing · alignment edges intentional · non-interactive elements not styled as interactive · text-on-image contrast holds at every breakpoint · passes the greyscale test.
- ☐ **P0** Form inputs (CRAFT-RULES C7): `type` + `inputmode` + `autocomplete` on every field (WCAG 1.3.5) · phone/card accept spaces, dashes, `+country` · numbers chunked · multi-step flows show progress.
- ☐ **P0** Choice & reach (CRAFT-RULES C12): exactly one primary CTA per view · nav 5–7 top-level · primary CTA is the largest interactive element in its band · mobile primary action in the bottom third · destructive actions not adjacent to the primary. Countable — count them.
