...
- [ ] **Grouping floor (CRAFT-RULES C13 — Gestalt)** — spacing **inside** a group is visibly tighter than spacing **between** groups (labels sit closer to their own field than to the next) · every alignment edge intentional · nothing non-interactive styled like the interactive things · text over imagery holds contrast at **every** breakpoint. **Greyscale test:** with color removed, the groupings and hierarchy still read.
- [ ] **Form input floor (CRAFT-RULES C7)** — every input carries `type`, `inputmode` and `autocomplete` (WCAG 1.3.5) · phone/card fields accept spaces, dashes and `+country` — normalise server-side, never reject on formatting · numbers chunked into groups · long forms sectioned · nothing must be carried in memory between steps · multi-step flows show progress, never starting at zero.
- [ ] **Choice & reach floor (CRAFT-RULES C12 — Hick's + Fitts's)** — **exactly one primary CTA per view** (secondary actions styled secondary) · top-level nav **5–7 items**, group beyond that · form asks the minimum that allows follow-up · the primary CTA is visibly the largest interactive element in its band · on mobile the primary action sits in the bottom third (sticky bar), never alone in a top corner · destructive actions are smaller and never adjacent to the primary · adjacent tap targets keep ~8px clear space · where options can't be cut, **group and label them rather than hiding them**, and pre-select a sensible default.
- [ ] **Convention floor (CRAFT-RULES C11 — Jakob's Law)** — logo top-left linking home · **every phone number a `tel:` link and every email a `mailto:` link**, in nav, announcement bar, sticky mobile CTA, contact page and footer · close controls top-right + `Esc` · body links visibly links, not color-only · form labels above inputs and persistent (never placeholder-as-label) · errors beside their field · skip-to-content first in tab order · back button unhijacked · standard icons keep standard meanings. At most **one** deliberate exception per build, named in the Compliance Report.
